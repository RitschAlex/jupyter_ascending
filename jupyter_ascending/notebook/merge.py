...
def opcode_merge_cell_contents(
    ...
    for (
        ...
        if opcode in {"replace", "insert"}:
            cells_to_update_text.extend(
                range(updated_start_idx, updated_final_idx))

        if opcode not in {"delete"}:
            current_cells_consumed.update(
                range(current_start_idx, current_final_idx))

        opcode_results.append(
            OpCodeAction(OpCodes(opcode), current_start_idx, current_final_idx,
                         updated_start_idx, updated_final_idx))

    current_cells_remaining = set(range(
        len(raw_current_contents))) - current_cells_consumed

    for updated_cell_index in cells_to_update_text:
        closest_match = get_close_matches(
            raw_updated_contents[updated_cell_index],
            [raw_current_contents[x] for x in current_cells_remaining],
            n=1)
...
def merge_cell_contents(
        ...
        for updated_cell in updated_cell_stack:
            if current_cell.source == updated_cell.source:
                ...
                if current_cell.index != updated_cell.index:
                    movements.append(
                        Movement(previous=current_cell.index,
                                 current=updated_cell.index))

                final_cells.append(
                    attr.evolve(current_cell, index=updated_cell.index))
    ...
    distance_between_cells: Dict[JupyterCell,
                                 List[CellDistance]] = defaultdict(list)
    for current_cell in current_cell_stack:
        ...
        for updated_cell in updated_cell_stack:
            ...
            distance = distancer.find_distance(current_cell.joined_source,
                                               updated_cell.joined_source)
            distance_between_cells[current_cell].append(
                CellDistance(distance, updated_cell))

        distance_between_cells[current_cell] = list(
            sorted(distance_between_cells[current_cell],
                   key=distancer.sort_function))
    ...
    while current_cell_stack and distance_between_cells and updated_cell_stack:
        sorted_cell_list = sorted(current_cell_stack,
                                  key=find_most_likely_cell)
        current_cell = sorted_cell_list[0]

        cell_distances = distance_between_cells.pop(current_cell)
        best_updated_cell = next(x.cell for x in cell_distances
                                 if x.cell in updated_cell_stack)
        ...
        evolved_cell = attr.evolve(current_cell,
                                   index=best_updated_cell.index,
                                   source=best_updated_cell.source)
    ...
    return evolve_notebook_cells(
        current_notebook, final_cells), CellMovements(movements=movements)
